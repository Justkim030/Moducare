...
function handleList(req, res) {
  ...
  let sql = `SELECT i.id, i.created, i.title, i.description, i.status, i.severity, i.employee_id, e.name as reporter_name FROM incidents i LEFT JOIN employees e ON e.id = i.employee_id WHERE 1=1`;
  const args = [];
...
function handleGet(req, res) {
  ...
  db.get(`SELECT i.id, i.created, i.title, i.description, i.status, i.severity, i.employee_id, e.name as reporter_name FROM incidents i LEFT JOIN employees e ON e.id = i.employee_id WHERE i.id = ?`, [id], (err, row) => {
    if (err) {
...
function handleCreate(req, res) {
  ...
  req.on('end', () => {
    try {
      ...
      const { title, description, severity, status, employee_id } = p;
...
function handleUpdate(req, res) {
  ...
  req.on('end', () => {
    try {
      const p = JSON.parse(body || '{}');
      const { title, description, status, severity, employee_id } = p;

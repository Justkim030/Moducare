...
function executeSchemaMigration() {
  ...
  db.serialize(() => {
    ...
    db.run(`DROP TABLE IF EXISTS pharmacy_dispensing`);
    db.run(`DROP TABLE IF EXISTS lab_orders`);
    db.run(`DROP TABLE IF EXISTS encounters`);
    ...
    // 5c. Lab Orders & Results
    db.run(`CREATE TABLE lab_orders (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      patient_id TEXT NOT NULL,
    ...
    // 5d. Pharmacy / ART Dispensing
    db.run(`CREATE TABLE pharmacy_dispensing (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      patient_id TEXT NOT NULL,
      encounter_id INTEGER,
    ...
    // 11b. Internal Incidents (Inheritance subtype)
